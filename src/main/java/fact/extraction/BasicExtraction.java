 ...
 * beginning with the half heigth position The resulting photoncharge is
...
public class BasicExtraction implements Processor {
    ...
    @Parameter(description = "key to the data array")
    ...
    public Data process(Data input) {
        ...
        for (int pix = 0; pix < npix; pix++) {
            positions[pix] = calculateMaxPosition(pix, startSearchWindow, startSearchWindow + rangeSearchWindow, roi,
                    data);
            mPositions[pix] = new IntervalMarker(positions[pix], positions[pix] + 1);

            int halfHeightPos = calculatePositionHalfHeight(pix, positions[pix], positions[pix] - rangeHalfHeightWindow,
                    roi, data);
