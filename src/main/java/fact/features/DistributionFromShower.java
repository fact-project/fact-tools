...
public Data process(Data input) {
    ...
    RealMatrix covarianceMatrix = calculateCovarianceMatrix(showerPixel, wheightsArray, cog);
    ...
    input.put("Ellipse", new EllipseOverlay(cog[0] , cog[1], width , length  , delta));
    ...
    public double calculateSkewness(double mean, double[] values, double[] weights){
        double sumWeights = 0;
        double skewness = 0;
    ...
    public double calculateKurtosis(double mean, double[] values, double[] weights){
        double sumWeights = 0;
        double kurtosis = 0;
        for (int i = 0; i < values.length; i++) {
            sumWeights +=weights[i];
    ...
    public double[] calculateCog(double[] weights, int[] showerPixel, double size){
        ...
        {
            cog[0]            += weights[pix] * pixelMap.getPixelFromId(pix).getXPositionInMM();
            cog[1]            += weights[pix] * pixelMap.getPixelFromId(pix).getYPositionInMM();
        }
    ...
    public RealMatrix calculateCovarianceMatrix(int[] showerPixel, double[] wheightsArray, double[] cog) {
        ...
        for (int pix: showerPixel )
        {
            ...
            variance_xx            += wheightsArray[pix] * (posx - cog[0]) * (posx - cog[0]);
            variance_yy            += wheightsArray[pix] * (posy - cog[1]) * (posy - cog[1]);
            covariance_xy          += wheightsArray[pix] * (posx - cog[0]) * (posy - cog[1]);
        }
