...
public class GaussianFit implements StatefulProcessor {
    ...
    @Parameter(required = false, description = "Base name for the output keys", defaultValue = "gaussian_fit")
    private String outputKey = "gaussian_fit";
    ...
    public Data process(Data data) {
        ...
        data.put(outputKey + "_r", r);
        data.put(outputKey + "_x", x);
        data.put(outputKey + "_y", y);
        data.put(outputKey + "_sigma", sigma);

        if (!r.isNaN()) {
            data.put(outputKey + "_overlay_1", new EllipseOverlay(x, y, r + sigma, r + sigma, 0));
            data.put(outputKey + "_overlay_2", new EllipseOverlay(x, y, r - sigma, r - sigma, 0));
        }
        return data;
    }
