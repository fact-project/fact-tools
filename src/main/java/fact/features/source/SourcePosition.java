...
/**
 * This calculates the position of the source in the camera. The Telescope
 * usually does not look directly at the source but somewhere close by. That
 * means the image of the source projected by the mirrors onto the camera is not
 ...
 *
 * This processor handles a handful of different tasks. It can calculate the
 * sourceposition in the camera for some fixed celestial coordinates (e.g. In
 ...
 * the datastream.
 *
 * For older ceres versions you can simply specify fixed X and Y coordinates in
 * the camera plane.
 *
...
public class SourcePosition implements StatefulProcessor {
    ...
    // flag which indicates whether were are looking at montecarlo files which
    // have a wobble position
    ...
    /**
     * Here we check whether an auxservice has been set or some fixed
     * coordinates have been provided in the .xml. If any of the parameters
     * sourceZdKey,sourceAzKey,pointingZdKey,pointingAzKey are set then all need
     * to be set.
     */
    @Override
    public void init(ProcessContext arg0) throws Exception {
        ...
        if (!(sourceZdKey == null && sourceAzKey == null && pointingZdKey == null && pointingAzKey == null)) {
            if (sourceZdKey != null && sourceAzKey != null && pointingZdKey != null && pointingAzKey != null) {
                ...
                log.warn("Using zd and az values from the item item");
            } else {
            ...
            log.error(
                    "You have to provide fixed sourceposition coordinates X and Y, or specify position keys, or specify the auxService.");
            throw new IllegalArgumentException();
        }
    }
    ...
    /**
     * The process method adds the azimuth and zenith values for the pointing,
     * tracking and source position. It also adds an overlay to the item so the
     * position can be displayed in the viewer.
     */
    @Override
    public Data process(Data item) {
        ...
        if (hasMcWobblePosition) {
            ...
            // Due to the fact, that Ceres handle the coordinate in a different
            // way, we have to undo
        ...
        try {
            ...
            DateTime timeStamp = new DateTime((long) ((eventTime[0] + eventTime[1] / 1000000.) * 1000),
                    DateTimeZone.UTC);
            // the source position is not updated very often. We have to get the
            // point from the auxfile which
            // was written earlier to the current event
            AuxPoint sourcePoint = auxService.getAuxiliaryData(AuxiliaryServiceName.DRIVE_CONTROL_SOURCE_POSITION,
                    timeStamp, earlier);

            // We want to get the tracking point which is closest to the current
            // event.
            AuxPoint trackingPoint = auxService.getAuxiliaryData(AuxiliaryServiceName.DRIVE_CONTROL_TRACKING_POSITION,
                    timeStamp, closest);
    ...
    public double julianDayToGmst(double julianDay) throws IllegalArgumentException {
        ...
        double gmst0Hours = (24110.54841 + 8640184.812866 * T + 0.093104 * Math.pow(T, 2) - 0.0000062 * Math.pow(T, 3))
                / 3600;
    ...
    /**
     * This is an adaption of the C++ Code by F.Temme. This method calculates
     * Azimuth and Zenith from right ascension, declination and the time in gmst
     * format.
     * 
     * @param ra
     *            in decimal Archours (e.g. 5h and 30 minutes : ra = 5.5)
     * @param dec
     *            in decimal degrees (e.g. 21 degrees and 30 arcminutes : zd =
     *            21.5)
     * @param gmst
     *            the Eventtime of the current event in gmst format
     * @return an array of length 2 containing {azimuth, zenith} in degree, not
     *         null;
     */
    public double[] getAzZd(double ra, double dec, double gmst) {
        if (ra >= 24.0 || ra < 0.0 || dec >= 360.0 || dec < 0) {
            throw new RuntimeException(
                    "Ra or Dec values are invalid. They should be given in decimal Archours and decimal degree");
        }
    ...
    /**
     * Returns position of the source in the camera in [mm] from the given
     * pointing Azimuth and Zenith Code by F.Temme
     * 
     * @param pointingAz
     * @param pointingZe
     * @param sourceAz
