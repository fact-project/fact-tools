...
public class DrsCalibration implements StatefulProcessor {
    ...
    final static String[] drsKeys = new String[] { "RunNumberBaseline", "RunNumberGain", "RunNumberTriggerOffset",
            "BaselineMean", "BaselineRms", "GainMean", "GainRms", "TriggerOffsetMean", "TriggerOffsetRms" };
        ...
        try {
            ...
            for (String key : drsKeys) {
                if (!drsData.containsKey(key)) {
                    throw new RuntimeException("DRS data is missing key '" + key + "'!");
                }
            }

            this.drsBaselineMean = (float[]) drsData.get("BaselineMean");
            this.drsBaselineRms = (float[]) drsData.get("BaselineRms");
            this.drsTriggerOffsetMean = (float[]) drsData.get("TriggerOffsetMean");
            this.drsTriggerOffsetRms = (float[]) drsData.get("TriggerOffsetRms");
    ...
    public double[] applyDrsCalibration(double[] data, double[] destination, short[] startCellVector) {
        ...
        for (int pixel = 0; pixel < 1440; pixel++) {
            for (int slice = 0; slice < roi; slice++) {
                ...
                offsetPos = pixel * drsBaselineMean.length / 1440 + ((slice + start) % (drsBaselineMean.length / 1440));

                triggerOffsetPos = pixel * drsTriggerOffsetMean.length / 1440 + slice;
    ...
    public void init(ProcessContext processContext) throws Exception {
        ...
        // log.error("Url and Service are not set. You need to set one of
        // those");
    ...
    public void setUrl(SourceURL url) {
        log.info("Setting DRS data url to {}", url);
        this.url = url;
    }
