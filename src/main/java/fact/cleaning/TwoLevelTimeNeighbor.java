...
public class TwoLevelTimeNeighbor extends BasicCleaning implements Processor{
    ...
    public Data process(Data input) {
        ...
        if(showerPixel.size() > 0){
            ...
            PixelSet allpixle = new PixelSet();
            ...
            PixelSet pixelSet5 = new PixelSet();
            for (CameraPixel pix: cleanedPixelSet.set) {
                if (maxPhotoncharge * 0.5 < photonCharge[pix.id]) {
                    pixelSet5.addById(pix.id);
                }
            }
            input.put(outputKey + "05", pixelSet5);

            PixelSet pixelSet2 = new PixelSet();
            for (CameraPixel pix: cleanedPixelSet.set) {
                if (maxPhotoncharge * 0.2 < photonCharge[pix.id]) {
                    pixelSet2.addById(pix.id);
                }
            }
            input.put(outputKey + "02", pixelSet2);

            PixelSet pixelSet25 = new PixelSet();
            for (CameraPixel pix: cleanedPixelSet.set) {
                if (maxPhotoncharge * 0.25 < photonCharge[pix.id]) {
                    pixelSet25.addById(pix.id);
                }
            }
            input.put(outputKey + "025", pixelSet25);

            PixelSet pixelSet3 = new PixelSet();
            for (CameraPixel pix: cleanedPixelSet.set) {
                if (maxPhotoncharge * 0.3 < photonCharge[pix.id]) {
                    pixelSet3.addById(pix.id);
                }
            }
            input.put(outputKey + "03", pixelSet3);

            PixelSet pixelSet4 = new PixelSet();
            for (CameraPixel pix: cleanedPixelSet.set) {
                if (maxPhotoncharge * 0.4 < photonCharge[pix.id]) {
                    pixelSet4.addById(pix.id);
                }
            }
            input.put(outputKey + "04", pixelSet4);

            PixelSet pixelSet1 = new PixelSet();
            for (CameraPixel pix: cleanedPixelSet.set) {
                if (maxPhotoncharge * 0.1 < photonCharge[pix.id]) {
                    pixelSet1.addById(pix.id);
                }
            }
            input.put(outputKey + "01", pixelSet1);
        }
