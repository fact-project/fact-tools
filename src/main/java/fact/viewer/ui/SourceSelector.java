...
public class SourceSelector extends JPanel {
	...
	public void setEvent(Data event) {
		...
		for (final String key : event.keySet()) {
			...
			if((value.getClass().isArray() && EventUtils.toDoubleArray(value) != null && EventUtils.toDoubleArray(value).length > Constants.NUMBEROFPIXEL) 	
					|| 
					(value.getClass().isArray() && value.getClass().getComponentType().equals(IntervalMarker.class)) ) {
