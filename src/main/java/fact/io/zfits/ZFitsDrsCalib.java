...
public class ZFitsDrsCalib implements Processor {
	...
	public Data process(Data input) {
		...
		input.put(this.outputKey, floatData);
		return input;
	}

	public void applyDrsOffsetCalib(int numSlices, int numChannel, short[] data, short[] startCellData, short[] calibData) throws Exception {
		if (data==null || data.length != numSlices*numChannel)
			throw new Exception("The length of the data array is wrong.");
		if (startCellData==null || startCellData.length != numChannel)
			throw new Exception("The length of the startCellData should be the same as the number of Channel");
		if (calibData==null || calibData.length != 1024*numChannel)
			throw new Exception("The length of the calibData is not the same as 1024*numChannel");
		...
		for (int ch=0; ch<numChannel; ch++) {
			...
			for (int sliceNum=0; sliceNum<numSlices; sliceNum++) {
				...
				data[ch*numSlices+sliceNum] += calibData[curCell];
			}
		}
	}
	...
	public void initDrsCellOffset(SourceURL url) throws Exception {
		...
		if (!item.containsKey("OffsetCalibration"))
			throw new Exception("Missing OffsetCalibration");
